...
export class BitArray {
  ...
  set(index, value) {
    ...
    if (indexInLong >= 32) {
      ...
      this.data[startLongIndex * 2 + 1] =
        ((this.data[startLongIndex * 2 + 1] &
          ~(this.valueMask << indexInStartLong)) |
  ...
  buffer() {
    let buffer = new DataView(new ArrayBuffer(this.data.length * 4));
    for (let i = 0; i < this.data.length; i += 2) {
      // smartBuffer.writeUInt32BE(this.data[i + 1]);
      // smartBuffer.writeUInt32BE(this.data[i]);

      buffer.setUint32(i * 4, this.data[i + 1]);
      buffer.setUint32(i * 4 + 4, this.data[i]);
    }
