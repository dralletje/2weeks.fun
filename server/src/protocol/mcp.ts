...
import { text_component } from "./text-component.ts";
...
export let mcp = {
  ...
  string_with_maxlength: (length: number) =>
    wrap({
      protocol: with_varint_length(native.string) satisfies Protocol<string>,
      decode(internal) {
        if (internal.length > length) {
          throw new Error(
            `String too long while decoding: ${internal.length} > ${length}`
          );
        }
        return internal;
      },
      encode(value: string) {
        if (value.length > length) {
          throw new Error(
            `String too long while encoding: ${value.length} > ${length}`
          );
        }
        return value;
      },
    ...
    return {
      ...
      read: (buffer: Uint8Array) => {
        try {
          ...
          throw new Error(
            `Error while reading packet 0x${packet_id.toString(16)}: ${error}`,
            { cause: error }
          );
        }
      },
